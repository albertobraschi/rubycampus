...
class EntryTerm < ActiveRecord::Base  
  has_many :contacts
  ...
  NAMES_KEYS = self.find(:all, :conditions => [ "is_enabled = ? and start_date > ?", true, Time.zone.now ]).map do |s| 
  [s.name, s.id] 
  end 
...
#
# Table name: rubycampus_entry_terms
#
