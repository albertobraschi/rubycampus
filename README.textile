bq. THIS SOFTWARE IS CURRENTLY UNDER ACTIVE DEVELOPMENT AND THIS REPOSITORY IS A SNAPSHOT THAT IS NOT INTENDED FOR GENERAL USE. IT IS PURELY INTENDED FOR RUBYCAMPUS DEVELOPERS AND OTHER INTERESTED PARTIES FOLLOWING AND/OR CONTRIBUTING TOWARDS THE PROJECT.  

bq. このソフトウェアは現在開発中です。 またこのレポジトリは、一般の使用目的ではないスナップショットです。これは本来 RUBYCAMPUS の開発者たちとその他のそのプロジェクトへ貢献している方たちの対象のものです。

!https://rubycampus.org/r/screenshots/001.png!

h2. RubyCampus - Relationship and Fundraising Management for Higher Education       

bq. RubyCampus is designed to store and manage highly sensitive information regarding constituents of educational organizations, governmental and non-governmental agencies. Every effort has been made to help ensure RubyCampus meets and exceeds the standards set by regulatory bodies around the world regarding the storage and retrieval of sensitive information through the use of security and cryptographic mechanisms. 

bq. If you downloaded an archive of RubyCampus or obtained it from any other source other than an "official" repository such as "rubycampus.org":https://rubycampus.org we strongly recommend that you clone and use only official source for production purposes. This will also ensure your organization is receiving updates directly from the authors and that the source has not been negatively or maliciously compromised.

Trusted Mirrors ( synched with main repository at "rubycampus.org":https://rubycampus.org ):

* GitHub - git://github.com/rubycampus/rubycampus.git    
* Gitorious - git://gitorious.org/rubycampus/mainline.git 
* RubyForge - git://rubyforge.org/rubycampus.git

*See the instructions below, and also see the "RubyCampus Wiki":https://rubycampus.org/wiki/rubycampus for more details.*

h2. Overview

This README file refers to the RubyCampus relationship management and fundraising software designed to
address the needs of higher educational and non-profit organizations for the purposes of student recruitment, 
enrollment management and fundraising.

The purpose of this file is to enumerate the various versions of the RubyCampus along with the appropriate 
versions of related dependencies needed to successfully deploy.

h2. Minimal Requirements

The current version of the RubyCampus can be installed and executed on any Unix based or inspired operating
system such as Linux, FreeBSD and Apple OS X and other compatible operating systems.
 
We recommend the following deployment environment but you may choose any number of alternatives that are
perfectly suitable.

* Linux/BSD OS
* Apache2
* MySQL
* Recommended: 
** Phusion Passenger (mod_rails)
** Ruby 1.8.7 Enterprise Edition
** Gem 1.2.0   - Required
** Rails 2.1.0 - Required

You can also get free support from the RubyCampus open source community.
See "rubycampus.org":https:/rubycampus.org/

*Copyright (C) 2008 Kevin Aleman, RubyCampus LLC Japan. All rights reserved.*   

h2. License

See the file doc/LICENSE.